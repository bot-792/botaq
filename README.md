<p align="center">
  <img src="https://files.catbox.moe/oxz51r.jpg" width="250" alt="JITOSA Logo"/>
</p>

<h1 align="center">𝗝𝗜𝗧𝗢𝗦𝗔 𝗕𝗢𝗧 🇲🇦</h1>
<h3 align="center">بـوت واتـسـاب متـعدد الاجـهـزة بالـذكـاء الاصـطـناعي</h3>

---

<p align="center">
<a href="https://instagram.com/adam.__.98"><img src="https://img.shields.io/badge/INSTAGRAM-E4405F?style=for-the-badge&logo=instagram&logoColor=white" /></a>
<a href="https://chat.whatsapp.com/رابط_القروب_ديالك"><img src="https://img.shields.io/badge/GROUP%20WHATSAPP-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /></a>
</p>

<p align="center">
<a href="https://whatsapp.com/channel/0029VbCxraN7T8bbAyc2j31J"><img src="https://img.shields.io/badge/CHANNEL%20WHATSAPP-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /></a>
<a href="https://wa.me/212698498657"><img src="https://img.shields.io/badge/DEVELOPER%20WHATSAPP-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /></a>
</p>

<p align="center">
  <a href="https://github.com/mysto-off">
    <img src="https://img.shields.io/badge/DEVELOPER%20GITHUB-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

### 📝 <span style="color:#00D9FF">حول البوت</span>
JITOSA BOT هو بوت واتساب متعدد الأجهزة مبني على `Baileys` مع دعم الذكاء الاصطناعي. 
يدعم الأوامر السريعة، التحميل، والالعاب وكلشي ناضي 🚀

---

### ✨ <span style="color:#FFD93D">شرح المميزات</span>

| الميزة | الشرح |
| :---: | :--- |
| 📥 **Downloader** | تحميل من `Instagram` , `TikTok` , `YouTube` <br> `(MP3/MP4)`, `MediaFire` وغيرها |
| 🤖 **الذكاء البحث** | بحث ذكي بـ `GPT` , `Gemini` , `Deepseek` + بحث جوجل وصور |
| 🕌 **القسم الديني** | `أذكار` , `قرآن` , `أدعية` , `مواقيت الصلاة` |
| 🌤️ **الطقس و الأخبار** | حالة الطقس لأي مدينة + آخر الأخبار العالمية |
| 📢 **الابلاغ** | نظام ابلاغ عن المشاكل واقتراحات مباشرة للمطور |
| 👑 **الإدارة** | أدوات للأدمن مثل `kick` , `promote` , `hideTag` و إعدادات القروب |
| 🧑‍💻 **المطور** | أوامر خاصة بالمطور `eval` , `exec` , `broadcast` |
| 🎭 **مجموعة الملصقات** | `Sticker` , `Sticker Video` , `Sticker GIF` و إنشاء باكات |
| 🎮 **الترفيه** | أوامر ممتعة مثل `truth` , `dare` و ألعاب تفاعلية |
| 🔧 **أدوات** | `Stalker` , `TTS` , `QR` , `Translate` و أدوات أخرى مفيدة |

---

### 🔗 <span style="color:#58A6FF">Utilitas</span>

**شـرح طـريقة استخدم الأومر:**
```bash
ارسل الصورة + .ملصق
ارسل الفيديو + .ملصق
